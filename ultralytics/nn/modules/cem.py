...
class ConetextEnhancementModule(nn.Module):
    """
    section 4.2
    ...
    def __init__(self, in_channels: int = 3, sigma: int = 2):
        super().__init__()
    ...
    def _lowpass(self, x):
        """
        [in] x: input matrix of shape [B, C, H, W]
        [out] x3: output matrix of shape [B, C, H, W]
        """
        H, W = x.shape[2:]
        x1 = self.lowpass_conv(x)
        x2 = self.lowpass_pool(x)
    ...
    def _generate_heatmap(self, x):
        """
        [in] x: input matrix of shape [B, C, H, W]
        [out] out: heatmap of shape [B, H, W]
        """
        out = x.mean(dim=1)
        return out
    ...
    def _heatmap_softmax(self, heatmap):
        ...
        pad_y, pad_x = H % s, W % s
        pad_y_left, pad_y_right = np.floor(pad_y / 2), np.ceil(pad_y / 2)
        pad_x_left, pad_x_right = np.floor(pad_x / 2), np.ceil(pad_x / 2)
        padded_heatmap = F.pad(heatmap, (pad_y_left, pad_y_right, pad_x_left, pad_x_right))
        Hp, Wp = padded_heatmap.shape[2:]

        # convert heatmap to relative weight of patches
        patches = heatmap.view(B, Hp//s, s, Wp//s, s)
        patches_logits = torch.sum(torch.sum(patches, dim=-1), dim=-2)      # [B, Hp, Wp]
        patches_weight = F.softmax(patches_logits, dim=0)

        # since we have the padding information in this function, let's expand it back to it's intended shape here
        patches_weight_expanded = patches_weight(1, s)
    ...
    def _highpass(self, x):
        ...
        f_max = torch.max(x, dim=1, keepdim=True)                   # [B, 1, H, W]
        f_mean = torch.mean(x, dim=1, keepdim=True)                 # [B, 1, H, W]
        f_concat = torch.stack([f_max, f_mean], dim=1)              # [B, 2, H, W]
        x_s = F.sigmoid(self.spatial_attention_kernel(f_concat))    # [B, 1, H, W]

        # compute X_smile
        x_smile = (x_s[:, None, :, :] * self.highpass_kernel(x)) + x
        return x_smile


    def forward(self, x):
        # perform heatmap-guided and low-pass filter paths, the combine their results 
        lowpass_out = self._lowpass(x)
        heatmap_out = self._heatmap_softmax(self._generate_heatmap(x))
        x_frown = torch.mul(lowpass_out heatmap_out[:, None, :, :])
        
        # high-pass filter path
        x_bar = x_frown - x
        x_smile = self._highpass(x_bar)

        return x_bar + x_smile
